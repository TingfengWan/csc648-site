...
function validateFields() {
  ...
  for (let i = 0; i < fields.length - 1; i++) {
    ...
    if (field.id == "first-name" || field.id == "last-name") {
    ...
    else if (field.id == "school-role") {
      if (field.value.length == 0) {
        field.style.border = "1px solid red";
        errorMsg = "Please choose your school role.";
        valid = false;
      }
    }
      ...
      else if (password.style.border == "1px solid red") {
        field.style.border = "1px solid red";
      }
    }

    else if (field.id == "terms-and-cond") {
...
function validatePhoneNumber(phoneNumber) {
  ...
  let phone3 = /^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{4})$/;

  if (phoneNumber.match(phone1) || phoneNumber.match(phone2) || phoneNumber.match(phone3)) {
    return true;
  }
